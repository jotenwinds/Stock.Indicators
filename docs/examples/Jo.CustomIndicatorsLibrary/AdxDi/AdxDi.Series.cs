...
// AVERAGE DIRECTIONAL INDEX (SERIES) USING SMMA (OR RMA)
public static partial class Indicator
{
    internal static List<AdxDiResult> CalcAdxDi(
        this List<QuoteD> qdList,
        int lookbackPeriods, // TODO-REMOVE
        int adxSmoothingLength = 13,
        int diLength = 13,
        int threshold = 21)
    {
        // check parameter arguments
        ValidateAdxDi(lookbackPeriods);

        // initialize
        int length = qdList.Count;
        List<AdxDiResult> results = new(length);
    ...
    private static void ValidateAdxDi(
        int lookbackPeriods)
    {
        // check parameter arguments
        if (lookbackPeriods <= 1)
        {
            throw new ArgumentOutOfRangeException(nameof(lookbackPeriods), lookbackPeriods,
                "Lookback periods must be greater than 1 for AdxDi.");
        }
    }
}
